...
mooring_id='KF_12_13';
start_date=datenum('03-Oct-2012 18:00:00');
end_date=datenum('03-Sep-2013 11:00:00');
moor_lat_deg = 78; moor_lat_min = 57.728;
moor_lon_deg = 11; moor_lon_min = 48.428;
mooring_depth = 241;
...
sbe16p_sn       = {'5181' '6101'}; % if more than one use format {'1111' '1112'}
sbe16p_depth    = {'25.5' '43.5'}; % if more than 1 use format {'20' '30'}
...
% conductivity
off_sbe16p_t = [NaN;0]; % From intercomparison data 2016 
off_sbe16p_c = [NaN;0]; % From intercomparison data 2016
% Cal dates
sbe16p_p_cal_date = {'22-May-2007' '13-Jun-2012'};
sbe16p_t_cal_date = {'07-Jun-2007' '15-Jun-2012'};
sbe16p_c_cal_date = {'07-Jun-2007' '15-Jun-2012'};
% Other sensors
sbe16p_flc = [1;1]; % Fluorescence
sbe16p_flc_sn = {'2352' '1443'};
sbe16p_flc_type = {'Seapoint' 'WetLabs WETstar'};
sbe16p_flc_cal_date = {'08-Feb-2001' '21-Jun-2012'};
sbe16p_par = [1;1]; % PAR
sbe16p_par_sn = {'70191' '70466'};
sbe16p_par_type = {'Biospherical' 'Biospherical'};
sbe16p_par_cal_date = {'07-Aug-2008' '06-Aug-2012'};
...
sbe37_depth    = {  '33'  '228'}; % if more than 1 use format {'20' '30'}
% Pressure sensor: 0 = not attached, 1 = attached. If more than one sbe37 
% on the mooring use format [1;1]
sbe37_p = [1;1]; 
% Offsets determined after intercomparison casts. t = temperature, c =
% conductivity
off_sbe37_t = [0;0]; % From intercomparison 2014
off_sbe37_c = [0.1;0]; % From intercomparison 2014 (note for S/N 5509 the data looked better applying the offset found the following year - i.e. values falling between the two other CTDs nearby) 
% Cal
...
ml_sn       = {'1594' '1610' '1612' '1619' '1628' '1629' '1691'};
ml_depth    = {  '33' '43.5' '54.5' '78.5'  '112'  '147' '177.5'};
ml_cal_date = {'unknown' 'unknown' 'unknown' 'unknown' 'unknown' 'unknown' 'unknown'};
% Offsets from temp bath at SAMS Jun-12
off_ml_t = [0.01;0;0;-0.02;0.06;-0.01;-0.56];
