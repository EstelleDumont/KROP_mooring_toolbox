...
if exist('sbe16p_num','var')
    if sbe16p_num >0
        ...
        for j = 1:sbe16p_num
            ...
            if ~exist('off_sbe16p_p'); off_sbe16p_p=NaN(length(off_sbe16p_t),1); end
            if (isnan(off_sbe16p_t(j)) && isnan(off_sbe16p_c(j)) && isnan(off_sbe16p_p(j)))
                ...
                if off_sbe16p_t(j) ~=0 || off_sbe16p_c(j) ~=0 || off_sbe16p_p(j) ~=0
                    msg = [inst ' offsets T = ' num2str(off_sbe16p_t(j)) ' ; C = ' num2str(off_sbe16p_t(j)) ' ; P = ' num2str(off_sbe16p_p(j)) ' : applying offsets'];
                    disp(msg)
                    ...
                    if ~isnan(off_sbe16p_p(j))
                        DATA_OUT.pres = DATA_IN.pres + off_sbe16p_p(j);
                        DATA_OUT.depth = sw_dpth(DATA_OUT.pres,mooring_lat);
                    end
                    ...
                    % Figure 2: pressure + depth offset
                    if ~isnan(off_sbe16p_p(j))
                        fig2 = figure('units','normalized','Position',[0.01,0.05,0.98,0.87]);

                        p1=subplot(4,1,1);
                        hold on
                        plot(DATA_IN.mtime,DATA_IN.pres,'*-','color',[.6,.6,.6],'markersize',5);
                        plot(DATA_OUT.mtime,DATA_OUT.pres,'g*-','markersize',2)
                        grid on
                        datetick('x',12)
                        ylabel('Pressure (db)','fontsize',7)
                        set(gca,'fontsize',7)
                        lg=legend('Raw','Offset','location','EastOutside');
                        set(lg,'fontsize',7)
                        title([mooring_id '  ' inst],'interpreter','none','fontsize',10)
                        ...
                        p2=subplot(4,1,2);
                        hold on
                        plot(DATA_IN.mtime,DATA_IN.depth,'*-','color',[.6,.6,.6],'markersize',5)
                        plot(DATA_OUT.mtime,DATA_OUT.depth,'g*-','markersize',2)
                        grid on
                        datetick('x',12)
                        ...
                        % Save figure
                        fig_nm = [d_plot_o '\offset_' mooring_id '_' inst '_pressure'];
                        saveas(fig,fig_nm,'png')
                    end
...
if exist('sbe19p_num','var')
    if sbe19p_num >0
        ...
        for j = 1:sbe19p_num
            ...
            % Check if offsets are non-zero
            if ~exist('off_sbe19p_p'); off_sbe19p_p=NaN(length(off_sbe19p_t),1); end
            if (isnan(off_sbe19p_t(j)) && isnan(off_sbe19p_c(j)) && isnan(off_sbe19p_p(j)))
                ...
                % Check if offsets are non-zero
                if off_sbe19p_t(j) ~=0 || off_sbe19p_c(j) ~=0 || off_sbe19p_p(j) ~=0
                    msg = [inst ' offsets T = ' num2str(off_sbe19p_t(j)) ' ; C = ' num2str(off_sbe19p_t(j)) ' ; P = ' num2str(off_sbe19p_p(j)) ' : applying offsets'];
                    disp(msg)
                    ...
                    if ~isnan(off_sbe19p_p(j))
                        DATA_OUT.pres = DATA_IN.pres + off_sbe19p_p(j);
                        DATA_OUT.depth = sw_dpth(DATA_OUT.pres,mooring_lat);
                    end
...
if exist('hcat_num','var')
    if hcat_num >0
        ...
        for j = 1:hcat_num
            ...
            % Check if offsets are non-zero
            if ~exist('off_hcat_p'); off_hcat_p=NaN(length(off_hcat_t),1); end
            if isnan(off_hcat_t(j))
                ...
                % Check if offsets are non-zero
                if off_hcat_t(j) ~=0 || off_hcat_c(j) ~=0 || off_hcat_p(j) ~=0
                    msg = [inst ' offsets T = ' num2str(off_hcat_t(j)) ' ; C = ' num2str(off_hcat_t(j)) ' ; P = ' num2str(off_hcat_p(j)) ' : applying offsets'];
                    disp(msg)
                    ...
                    if ~isnan(off_hcat_p(j))
                        DATA_OUT.pres = DATA_IN.pres + off_hcat_p(j);
                        DATA_OUT.depth = sw_dpth(DATA_OUT.pres,mooring_lat);
                    end
...
if exist('sbe37_num','var')
    if sbe37_num >0
        ...
        for j = 1:sbe37_num
            ...
            % Check if offsets are non-zero
            if ~exist('off_sbe37_p'); off_sbe37_p=NaN(length(off_sbe37_t),1); end
            if (isnan(off_sbe37_t(j)) && isnan(off_sbe37_c(j)) && isnan(off_sbe37_p(j)))
                ...
                % Check if offsets are non-zero
                if off_sbe37_t(j) ~=0 || off_sbe37_c(j) ~=0 || off_sbe37_p(j) ~=0
                    msg = [inst ' offsets T = ' num2str(off_sbe37_t(j)) ' ; C = ' num2str(off_sbe37_t(j)) ' ; P = ' num2str(off_sbe37_p(j)) ' : applying offsets'];
                    disp(msg)
                    ...
                    if ~isnan(off_sbe37_p(j))
                        DATA_OUT.pres = DATA_IN.pres + off_sbe37_p(j);
                        DATA_OUT.depth = sw_dpth(DATA_OUT.pres,mooring_lat);
                    end
...
if exist('ml_num','var')
    if ml_num >0
        ...
        for j = 1:ml_num
            ...
            % Check if offsets are non-zero
            if ~exist('off_ml_p'); off_ml_p=NaN(length(off_ml_t),1); end
            if (isnan(off_ml_t(j)) && isnan(off_ml_p(j)))
