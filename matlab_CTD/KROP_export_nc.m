function KROP_export_nc(info_file)
...
%%% File 1: full res timeseries for each instrument

% Turn warning off for empty date strings (when no cal dat because sensor doesn't exist)
...
if ~isempty(nc_list)
    for n = 1:length(nc_list)
        if isempty(regexp(nc_list(n).name,'avg24h')) % don't delete averaged files
...
for ind_inst = 1:length(list_inst)
    ...
    ans_yn = 0;
    while ans_yn == 0
        ans_edt = input(['Do you want to edit any attributes for ' inst_type ' S/N ' inst_sn '? (y/n)     '],'s');
    ...
    eval(['cd ' I.d_nc])
    ncf = [I.mooring_id '_pro_' inst_str '_' inst_sn '.nc'];
    ...
    ga_tit = [I.mooring_id '_pro_' inst_str '_' inst_sn];
    if regexp(inst_type,'ESM1','ignorecase')
        ga_sum = ['Fluorescence data from ESM-1 logger at ' num2str(data_PRO.nominal_depth(1)) 'm on mooring ' I.mooring_id];
    else
        ga_sum = ['Post-processed CTD data from ' inst_str ' at ' num2str(data_PRO.nominal_depth(1)) 'm on mooring ' I.mooring_id];
    end
    ...
    if regexp(inst_type,'SBE16p')
        ...
        ATT = load([I.d_matlab '\KROP_data_att.mat']);
        
        % List all possible variables
        var_list = {'mtime','scan','nominal_depth','pres','depth',...
        ...
        for jj = 1:length(var_list)
            if isfield(data_PRO,var_list{jj})
                var_name = char(var_list{jj});
                if regexp(var_name(1:2),'mt')
                    % Convert matlab time to epoch time
                    var_data = (86400 * (data_PRO.mtime - datenum('01-Jan-1970 00:00:00')));
                else
                    eval(['var_data = data_PRO.' var_list{jj} ';'])
                end
                clear var_name
    ...
    elseif regexp(inst_type,'HCAT')
        ...
        % List all possible variables
        var_list = {'mtime','scan','nominal_depth','pres','depth','temp',...
    ...
    elseif regexp(inst_type,'SBE56')
        ...
        % List all possible variables
        var_list = {'mtime','nominal_depth','temp'};
        % List name of export variables
    ...
    elseif regexp(inst_type,'ESM1')
        ...
        % List all possible variables
        var_list = {'mtime','nominal_depth','flc_eng','flc'};
        % List name of export variables
    ...
    clear inst* ncf ATT data_PRO ga_* ans* def*
